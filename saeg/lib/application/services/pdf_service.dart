...
class PdfService {
  // Tracks whether a fallback font was used (can be checked after generate())
  bool usedFallbackFont = false;
  String? fontWarningMessage;

  Future<pw.Font> _loadFont() async {
    final assetPath = SystemConfig.contractFontAssetPath;
    try {
      ...
      // Try TraditionalArabic as first fallback
      try {
        final fallback = await rootBundle.load('assets/fonts/TraditionalArabic.ttf');
        usedFallbackFont = true;
        fontWarningMessage =
            'تعذر تحميل الخط ${SystemConfig.contractFontDisplayName}، تم استخدام Traditional Arabic بديلاً';
        return pw.Font.ttf(fallback);
      } catch (_) {
        // Final fallback: built-in Times font
        usedFallbackFont = true;
        fontWarningMessage =
            'تعذر تحميل الخطوط العربية، تم استخدام الخط الافتراضي';
  ...
  Future<pw.Font> _loadBoldFont() async {
    final assetPath = SystemConfig.contractFontBoldAssetPath;
    try {
      final fontData = await rootBundle.load(assetPath);
      return pw.Font.ttf(fontData);
    } catch (_) {
  ...
  // Fixed: correctly maps ContractPageFormat to PdfPageFormat
  ...
  Future<File> generate(Contract contract, {bool blankTemplate = false}) async {
    ...
    final fileName = blankTemplate
        ? 'قالب_فارغ_${contract.id}.pdf'
        : 'عقد_${contract.id}.pdf';
  ...
  List<pw.Widget> _buildContent(Contract contract, bool blank) {
    ...
    widgets.add(_divider());
    widgets.add(_text(
        'في هذا اليوم الـ ${blank ? '___________' : (contract.contractDate.isEmpty ? '...' : contract.contractDate)} في مدينة ${blank ? '___________' : (contract.city.isEmpty ? '...' : contract.city)}، المحافظة ${blank ? '___________' : (contract.governorate.isEmpty ? '...' : contract.governorate)}، تم التعاقد ما بين:'));
    ...
    if (contract.type == ContractType.inheritanceAgreement ||
        contract.type == ContractType.judicialInheritance) {
  ...
  pw.Widget _personBlock(Person p, bool blank) {
    ...
    return pw.Column(
      ...
      children: [
        ...
        if (p.hasPowerOfAttorney)
          _text('صفة التوقيع: وكالة بموجب الوكالة رقم ${v(p.poaNumber)} وتاريخ ${v(p.poaDate)}'),
      ],
    );
  }
  ...
  pw.Widget _propertyBlock(Contract c, bool blank) {
    ...
    return pw.Column(
      ...
      children: [
        ...
        if (p.isCommonShare)
          _text('الحصة الشائعة: ${ArabicTextHelpers.toArabicDigits(p.commonShareNumerator)} / ${ArabicTextHelpers.toArabicDigits(p.commonShareDenominator)}'),
  ...
  pw.Widget _heirsBlock(Contract c, bool blank) {
    ...
    return pw.Column(
      ...
      children: c.heirs.map((h) {
        ...
        final shares =
            blank ? '___________' : ArabicTextHelpers.toArabicDigits(h.shares);
  ...
  pw.Widget _clausesBlock(Contract c, bool blank) {
    String v(String val) =>
        blank ? '___________' : (val.isEmpty ? '...' : val);
    final price = blank
        ? '___________'
        : ArabicTextHelpers.toArabicDigits(c.payment.totalPrice);
    final paid = blank
        ? '___________'
        : ArabicTextHelpers.toArabicDigits(c.payment.paidAmount);
    final balance = blank
        ? '___________'
        : ArabicTextHelpers.toArabicDigits(c.payment.balance);
    final penalty = blank
        ? '___________'
        : ArabicTextHelpers.toArabicDigits(c.payment.penaltyAmount);
    final tax = blank
        ? '___________'
        : ArabicTextHelpers.toArabicDigits(
            (c.payment.totalPrice * 0.03).round());
    final clauses = <pw.Widget>[];

    clauses.add(_sectionTitle('البند الأول (البيع والثمن)'));
    clauses.add(_text(
        'باع البائع للمشتري العقار المذكور، بيعاً قطعياً، لقاء ثمن إجمالي قدره ($price) ${_currencyAr(c.payment.currency)}. قبض منه مبلغ ($paid) ${_currencyAr(c.payment.currency)}، والرصيد ($balance) ${_currencyAr(c.payment.currency)} يدفع بتاريخ ${v(c.payment.balanceDueDate)} ${c.payment.method == PaymentMethod.installments ? 'وفق جدول الدفعات المرفق' : ''}.'));
  ...
  pw.Widget _legalNotice() => pw.Padding(
        ...
        child: pw.Text(
          'تنبيه قانوني إلزامي: ${LegalPhrases.legalNotice}\n\n${LegalPhrases.twoCopies}',
  ...
  pw.Widget _signaturesBlock(Contract c, bool blank) {
    final sellerName = blank
        ? '___________'
        : (c.sellers.isNotEmpty ? c.sellers.first.fullName : '...');
    final buyerName = blank
        ? '___________'
        : (c.buyers.isNotEmpty ? c.buyers.first.fullName : '...');
    return pw.Column(
      children: [
        ...
        pw.Row(
          ...
          textDirection: pw.TextDirection.rtl,
          children: [
            _signatureBox(c.witnesses.isNotEmpty && !blank
                ? c.witnesses.first.fullName
                : '___________'),
            _signatureBox(c.witnesses.length > 1 && !blank
                ? c.witnesses[1].fullName
                : '___________'),
          ],
        ),
      ],
  ...
  pw.Widget _annexBlock(ContractAnnex annex, bool blank) => pw.Column(
        children: [
          ...
          pw.Container(
            ...
            child: pw.Text(
              'ملحق رقم ${annex.number}: ${annex.titleAr}',
              textDirection: pw.TextDirection.rtl,
              style: pw.TextStyle(
