...
class ArabicTextHelpers {
  ...
  static String formatDate(String input) {
    ...
    try {
      ...
      if (parts.length == 3) {
        return '${toArabicDigits(int.tryParse(parts[0]) ?? parts[0])}'
            '/${toArabicDigits(int.tryParse(parts[1]) ?? parts[1])}'
            '/${toArabicDigits(int.tryParse(parts[2]) ?? parts[2])}';
      }
      return input;
    } catch (_) {
      return input;
  ...
  // Static utility: simplify fraction and return Arabic digits
