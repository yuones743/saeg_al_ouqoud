...
class _QuickContractWizardState extends State<QuickContractWizard> {
  ...
  void dispose() {
    _pageCtrl.dispose();
    for (final c in [
      _dateCtrl, _cityCtrl, _sellerCtrl, _sellerIdCtrl, _sellerPhoneCtrl,
      _buyerCtrl, _buyerIdCtrl, _buyerPhoneCtrl, _registryCtrl, _zoneCtrl,
      _totalCtrl, _paidCtrl,
    ]) {
      c.dispose();
    }
    super.dispose();
  ...
  Future<void> _submit() async {
    ...
    provider.updateSeller(Person(
      id: 'seller_qw1',
      fullName: _sellerCtrl.text.isEmpty ? 'البائع' : _sellerCtrl.text,
      nationalId: _sellerIdCtrl.text,
      phone: _sellerPhoneCtrl.text,
      role: PersonRole.seller,
    ));
    provider.updateBuyer(Person(
      id: 'buyer_qw1',
      fullName: _buyerCtrl.text.isEmpty ? 'المشتري' : _buyerCtrl.text,
      nationalId: _buyerIdCtrl.text,
      phone: _buyerPhoneCtrl.text,
      role: PersonRole.buyer,
    ));
    provider.updateProperty(
        Property(registryNumber: _registryCtrl.text, zone: _zoneCtrl.text));
    ...
    if (!mounted) return;
    // Fixed: Navigator.push instead of pushReplacement – allows going back to wizard
    Navigator.push(
      context,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
          title: Text('معالج العقد السريع – ${_currentPage + 1}/$_totalPages')),
            ...
            Expanded(
              ...
              child: ElevatedButton.icon(
                icon: Icon(_currentPage == _totalPages - 1
                    ...
                    : Icons.arrow_back),
                label: Text(
                    _currentPage == _totalPages - 1 ? 'فحص العقد' : 'التالي'),
  ...
  Widget _page1TypeAndDate() => ListView(padding: const EdgeInsets.all(16), children: [
        const Text('الخطوة الأولى: نوع العقد والتاريخ',
        ...
        SectionCard(title: 'نوع العقد', icon: Icons.assignment, children: [
          DropdownButtonFormField<ContractType>(
            ...
            items: const [
              DropdownMenuItem(
                  value: ContractType.directSale, child: Text('بيع مباشر')),
              DropdownMenuItem(
                  value: ContractType.usufructSale,
                  child: Text('بيع انتفاع')),
              DropdownMenuItem(
                  value: ContractType.commonShareSale,
                  child: Text('بيع حصة شائعة')),
              DropdownMenuItem(
                  value: ContractType.settlement,
                  child: Text('صلح ووساطة')),
              DropdownMenuItem(
                  value: ContractType.promise, child: Text('وعد بالبيع')),
              DropdownMenuItem(
                  value: ContractType.partition,
                  child: Text('قسمة رضائية')),
            ],
        ...
        SectionCard(title: 'التاريخ والمكان', icon: Icons.place, children: [
          ArabicTextField(
              ...
              label: 'تاريخ العقد (يوم/شهر/سنة)'),
          ArabicTextField(
              controller: _cityCtrl, label: 'المدينة', required: true),
          Padding(
  ...
  Widget _page2Seller() =>
      ListView(padding: const EdgeInsets.all(16), children: [
        const Text('الخطوة الثانية: بيانات البائع',
            style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
        const SizedBox(height: 16),
        SectionCard(title: 'البائع', icon: Icons.person, children: [
          ArabicTextField(
              ...
              required: true),
          ArabicTextField(
              controller: _sellerIdCtrl, label: 'الرقم الوطني'),
          ArabicTextField(
  ...
  Widget _page3Buyer() =>
      ListView(padding: const EdgeInsets.all(16), children: [
        const Text('الخطوة الثالثة: بيانات المشتري',
            style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
        const SizedBox(height: 16),
        SectionCard(title: 'المشتري', icon: Icons.person_outline, children: [
          ArabicTextField(
              ...
              required: true),
          ArabicTextField(
              controller: _buyerIdCtrl, label: 'الرقم الوطني'),
          ArabicTextField(
  ...
  Widget _page4PropertyAndPrice() =>
      ListView(padding: const EdgeInsets.all(16), children: [
        const Text('الخطوة الرابعة: العقار والثمن',
            style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
        const SizedBox(height: 16),
        SectionCard(title: 'العقار', icon: Icons.home, children: [
          ArabicTextField(
              controller: _registryCtrl, label: 'رقم السجل العقاري'),
          ArabicTextField(
              controller: _zoneCtrl, label: 'المنطقة العقارية'),
        ]),
        SectionCard(title: 'الثمن', icon: Icons.attach_money, children: [
          ArabicTextField(
              controller: _totalCtrl,
              label: 'الثمن الإجمالي',
          ...
          Padding(
            ...
            child: DropdownButtonFormField<Currency>(
              ...
              items: const [
                DropdownMenuItem(
                    value: Currency.syp, child: Text('ليرة سورية (ل.س)')),
                DropdownMenuItem(
                    value: Currency.usd, child: Text('دولار أمريكي')),
                DropdownMenuItem(value: Currency.eur, child: Text('يورو')),
                DropdownMenuItem(
                    value: Currency.sar, child: Text('ريال سعودي')),
              ],
