          ...
          if (result != null)
            IconButton(
              ...
              onPressed: () => Navigator.push(
                context,
                MaterialPageRoute(builder: (_) => HeirsSummaryScreen(result: result!)),
              ),
            ),
        ],
      ),
      body: contract.heirs.isEmpty
          ? const Center(
              child: Padding(
                ...
                child: Text(
                  'لا يوجد ورثة. أضف ورثة من شاشة عقد الإرث أولاً.',
                  ...
                  Container(
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        if (result.hasAwl) _statChip('⚠️ عول', 'نعم'),
                        if (result.hasRadd) _statChip('🔄 رد', 'نعم'),
                      ],
                    ),
                  ),
                  if (result.warnings.isNotEmpty)
                    Container(
                      ...
                      child: Column(
                        ...
                        children: result.warnings.map((w) => Row(
                          children: [
                  ...
                  if (result.calculationSteps.isNotEmpty)
                    Container(
                      ...
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          const Text('📊 خطوات الحساب:', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12)),
                          const SizedBox(height: 4),
                          ...result.calculationSteps.map((s) => Padding(
                            padding: const EdgeInsets.only(bottom: 2),
                            child: Text('• $s', style: const TextStyle(fontSize: 11, color: Colors.blueGrey)),
                          )).toList(),
                        ],
                      ),
                    ),
                  ...
                  child: ListView.builder(
                    ...
                    itemBuilder: (_, index) {
                      if (result == null) {
                        ...
                        return Card(
                          child: ListTile(
                            title: Text(heir.person.fullName.isEmpty ? 'وارث ${index + 1}' : heir.person.fullName),
                            subtitle: Text(heir.relation),
                          ),
                        );
                      }
                      final share = result.shares[index];
                      return Card(
                        color: share.isExcluded ? Colors.red.shade50 : _shareColor(share.shares),
                        child: Padding(
                            ...
                            children: [
                              Expanded(
                                child: Column(
                                  ...
                                  children: [
                                    Text(
                                      share.heirName.isEmpty ? 'وارث ${index + 1}' : share.heirName,
                                      style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 14),
                                    ),
                                    Text(share.relation, style: const TextStyle(fontSize: 12, color: Colors.grey)),
                                    if (share.isExcluded)
                                      Text(
                                        'محروم: ${share.exclusionReason}',
                                        style: const TextStyle(fontSize: 11, color: Colors.red),
                                      ),
                                    if (share.shareCategory.isNotEmpty)
                                      Text(
                                        'الفئة: ${share.shareCategory}',
                              ...
                              if (!share.isExcluded) ...[
                                Column(
                                  ...
                                  children: [
                                    Text(
                                      '${ArabicTextHelpers.toArabicDigits(share.shares)} / ${ArabicTextHelpers.toArabicDigits(result.totalShares)}',
                                      style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 15),
                                    ),
                                    Text(share.fractionLabel, style: const TextStyle(fontSize: 12, color: Colors.grey)),
                                    Text(
                                      '${(share.fraction * 100).toStringAsFixed(2)}%',
