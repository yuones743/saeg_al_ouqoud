...
class _InheritanceContractScreenState extends State<InheritanceContractScreen> {
  ...
  @override
  void dispose() {
    for (final c in [_deceasedNameCtrl, _deceasedFatherCtrl, _deceasedMotherCtrl, _dateCtrl, _cityCtrl, _registryCtrl, _zoneCtrl]) { c.dispose(); }
    for (final h in _heirs) { h.nameCtrl.dispose(); h.sharesCtrl.dispose(); }
    super.dispose();
  }
  ...
  void _pushDataToProvider() {
    ...
    provider.updateSeller(Person(id: 'deceased_1', fullName: _deceasedNameCtrl.text, fatherName: _deceasedFatherCtrl.text, motherName: _deceasedMotherCtrl.text, role: PersonRole.seller, isDeceased: true));
    provider.updateBuyer(const Person(id: 'buyer_heir', fullName: 'الورثة', role: PersonRole.buyer));
    provider.updateProperty(Property(registryNumber: _registryCtrl.text.isEmpty ? 'إرث' : _registryCtrl.text, zone: _zoneCtrl.text, isAmiriaLand: _propIsAmiriaLand));
    ...
    for (var i = 0; i < _heirs.length; i++) {
      ...
      provider.addHeir(Heir(
        person: Person(id: 'heir_$i', fullName: entry.nameCtrl.text, role: PersonRole.heir, isMinor: entry.isMinor, isMissing: entry.isMissing, isExpatriate: entry.isExpatriate),
        shares: int.tryParse(entry.sharesCtrl.text) ?? 0,
        isKiller: entry.isKiller, isApostate: entry.isApostate,
        isPrisoner: entry.isPrisoner, isIntersex: entry.isIntersex,
        isPregnant: entry.isPregnant, relation: entry.relation,
      ));
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          TextButton.icon(icon: const Icon(Icons.calculate, color: Colors.white),
            label: const Text('حاسبة الأسهم', style: TextStyle(color: Colors.white, fontSize: 12)),
            onPressed: _openCalculator),
        ],
      ),
      body: ListView(padding: const EdgeInsets.all(16), children: [
        SectionCard(title: 'بيانات المتوفى', icon: Icons.person_off, children: [
          ...
          if (_willExceedsThird)
            ArabicSwitch(label: 'موافقة الورثة على الوصية', value: _willHasHeirConsent, onChanged: (v) => setState(() => _willHasHeirConsent = v)),
        ]),
        ...
        SectionCard(title: 'الورثة', icon: Icons.people, children: [
          ..._heirs.asMap().entries.map((e) => _heirTile(e.key, e.value)),
          TextButton.icon(icon: const Icon(Icons.person_add), label: const Text('إضافة وارث'), onPressed: _addHeir),
        ]),
        const SizedBox(height: 16),
        Row(children: [
          Expanded(child: OutlinedButton.icon(icon: const Icon(Icons.pie_chart), label: const Text('حساب الأسهم'), onPressed: _openCalculator)),
          const SizedBox(width: 8),
          Expanded(child: PrimaryActionButton(icon: Icons.search, label: 'فحص العقد', onPressed: _submit)),
        ]),
  ...
  Widget _heirTile(int index, _HeirEntry entry) => Card(
    ...
    child: Padding(padding: const EdgeInsets.all(8),
      child: Column(children: [
        Row(children: [
          Expanded(child: ArabicTextField(controller: entry.nameCtrl, label: 'اسم الوارث')),
          const SizedBox(width: 6),
          SizedBox(width: 70, child: ArabicTextField(controller: entry.sharesCtrl, label: 'أسهم', type: TextInputType.number)),
          IconButton(icon: const Icon(Icons.delete, color: Colors.red, size: 20), onPressed: () => _removeHeir(index)),
        ]),
        const SizedBox(height: 6),
        DropdownButtonFormField<String>(
          value: entry.relation.isEmpty ? null : entry.relation,
          decoration: const InputDecoration(labelText: 'صلة القرابة', border: OutlineInputBorder(), isDense: true),
          items: const [
